...
Like B<installcheck>, but invokes B<psql>. Input is read from stdin.
...
=item B<debian/control.in:>
  ...
  Build-Depends: debhelper, postgresql-all (>= 217~)
...
the B<psql> action were introduced in postgresql-server-dev-all (>= 217~).
